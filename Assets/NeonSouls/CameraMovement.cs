...
public class CameraMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float appliedXSens = sensitivity;
        float appliedYSens = sensitivity;

        // Quaternion * Quaternion is the same as applying rotation from second to first
        Quaternion cameraRotation = followTarget.transform.rotation *= Quaternion.AngleAxis(playerController.Look.x * appliedXSens * Time.deltaTime, Vector3.up);

        cameraRotation *= Quaternion.AngleAxis(-playerController.Look.y * appliedYSens * Time.deltaTime, Vector3.right);
